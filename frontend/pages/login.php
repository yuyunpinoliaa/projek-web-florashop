...
<div class="relative z-10 w-full max-w-[440px] fade-in">
    <div class="glass-panel rounded-[32px] p-6 sm:p-10 shadow-[0px_4px_20px_rgba(244,114,182,0.08)] border border-white/40">
        ...
        <div class="flex p-1 bg-secondary-fixed/30 rounded-full mb-8">
            <a href="login.php" class="flex-1 text-center py-2 px-4 rounded-full font-label-md text-label-md transition-all duration-300 bg-white text-primary shadow-sm block">Sign In</a>
            <a href="register.php" class="flex-1 text-center py-2 px-4 rounded-full font-label-md text-label-md transition-all duration-300 text-secondary hover:text-primary block">Create Account</a>
        </div>
        ...
        <?php if(isset($_SESSION['success'])): ?>
            <div class="mb-6 p-3 bg-tertiary-container text-on-tertiary-container rounded-lg text-center text-sm font-medium">
                <?php echo $_SESSION['success']; unset($_SESSION['success']); ?>
            </div>
        <?php endif; ?>

        <form class="space-y-6 transition-opacity duration-300" action="../../backend/auth/proses_login.php" method="POST">
            <div class="space-y-2">
                <label class="font-label-md text-label-md text-secondary ml-1" for="email">Email Address</label>
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline text-[20px]">mail</span>
                    <input class="w-full pl-[48px] pr-4 py-3 bg-white/50 border border-outline-variant/40 rounded-xl focus:ring-2 focus:ring-primary-container focus:border-primary-container outline-none transition-all placeholder:text-outline/50 font-body-md text-body-md" id="email" name="email" placeholder="hello@example.com" type="email" required/>
                </div>
            </div>
            <div class="space-y-2">
                <div class="flex justify-between items-center px-1">
                    <label class="font-label-md text-label-md text-secondary" for="password">Password</label>
                    <a class="text-primary font-label-sm text-label-sm hover:underline" href="#">Forgot?</a>
                </div>
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline text-[20px]">lock</span>
                    <input class="w-full pl-[48px] pr-[48px] py-3 bg-white/50 border border-outline-variant/40 rounded-xl focus:ring-2 focus:ring-primary-container focus:border-primary-container outline-none transition-all placeholder:text-outline/50 font-body-md text-body-md" id="password" name="password" placeholder="••••••••" type="password" required/>
                </div>
            </div>
            <div class="flex items-center space-x-2 px-1">
                <input class="w-4 h-4 rounded text-primary border-outline focus:ring-primary-container" id="remember" name="remember" type="checkbox"/>
                <label class="font-label-sm text-label-sm text-secondary" for="remember">Remember me for 30 days</label>
            </div>
